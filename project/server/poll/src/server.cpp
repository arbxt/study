...
TcpServer::TcpServer(unsigned short port) : port_(port), listen_fd_(-1) {}
...
bool TcpServer::start() {
  ...
  // 设置非阻塞
  if (!setFdnonBlocking(listen_fd_)) {
    close(listen_fd_);
    listen_fd_ = -1;
    return false;
  }
   ...
   * 绑定 IP 和端口。
   */
  if (bind(listen_fd_, reinterpret_cast<sockaddr *>(&server_addr),
...
int TcpServer::accept_client() {
  ...
  if (client_fd == -1) {
    ...
    // std::cerr << "accept 失败: " << std::strerror(errno) << std::endl;
    return -1;
  }

  std::cout << "客户端已连接: " << inet_ntoa(client_addr.sin_addr) << ":"
            << ntohs(client_addr.sin_port) << ", client_fd = " << client_fd
            << std::endl;

  return client_fd;
}

int TcpServer::listen_fd() const { return listen_fd_; }